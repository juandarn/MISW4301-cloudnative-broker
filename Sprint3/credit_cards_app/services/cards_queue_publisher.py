...
class CardsQueuePublisher:
    """
    Publisher seguro:
    - off (default): hace no-op y retorna un dict simulado.
    - sqs: publica en SQS usando CARDS_QUEUE_URL.
    """
    def __init__(self, queue_url: str | None = None, client: Any | None = None):
        ...
        if self.provider == "sqs":
            ...
            if boto3 is None and client is None:
                raise RuntimeError("boto3 is required to publish messages")
    ...
    def publish(self, message: dict[str, Any]) -> dict[str, Any]:
        if self.provider == "off":
            # no-op pero no revienta
            return {"status": "noop", "message": message}
        # SQS
