...
class TrueNativeWebhookSchema(Schema):
    RUV = fields.String(required=True)
    score = fields.Raw(required=True)       # puede llegar como "100" (string) o 100 (num)
    verifyToken = fields.String(required=True)

    class Meta:
        unknown = INCLUDE  # no reventar si llegan campos extra
